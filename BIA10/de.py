    ...
    def differentialEvolution(self,funkce):
        ...
        while g < self.Gmaxim:
            ...
            for i, x in enumerate(actual_pop):
                ...
                rand = np.random.randint(0, self.NP)
                for j in range(self.dimensions):
                    if np.random.uniform() < self.CR or j == rand:
                        u[j] = v[j]
                    else:
                        u[j] = actual_pop[i][j]
                f_u = funkce.useFn(u,True)

                if f_u <= funkce.useFn(actual_pop[i],True):
                     new_pop[i] = u
            actual_pop = new_pop
            
            self.draw_evolution.append(new_pop)
            g += 1
...
# solution = Solution(-500,500,10,50,30)
# sf=SchwefelFunction()
# sf.createn(30)
# solution.differentialEvolution(sf)


# solution = Solution(-5,10,10,50,30)
# sf=RosenbrockFunction()
# sf.createn(30)
# solution.differentialEvolution(sf)

# solution = Solution(-5.12,5.12,10,50,30)
# sf=RastriginFunction()
# sf.createn(30)
# solution.differentialEvolution(sf)


# solution = Solution(-600,600,10,50,30)
# sf=GriewankFunction()
# sf.createn(30)
# solution.differentialEvolution(sf)


# solution = Solution(-10,10,10,50,30)
# sf=LevyFunction()
# sf.createn(30)
# solution.differentialEvolution(sf)


# solution = Solution(0,np.pi,10,50,30)
...
# solution = Solution(-32.768,32.768,10,50,30)
# sf=AckleyFunction()
# sf.createn(30)
# solution.differentialEvolution(sf)
