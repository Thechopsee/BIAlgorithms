...
class Function:
    ...
    def createn(self,n):
        self.n=n
        for x in range(n):
            self.prostory.append(np.arange(self.lb, self.ub, 0.1))
    def reset(self):
        self.maxohod=3000
...
class AckleyFunction(Function):
    ...
    def useFn(self,params,ign):
        ...
        soucetmocnin=0
        soucetcosinu=0
        f=lambda sm,cs:-20.0 * np.exp(-0.2 * np.sqrt(0.5 * sm))-np.exp(0.5 * cs) + np.e + 20
        for x in params:
                soucetmocnin+=x**2
                soucetcosinu+=np.cos(2 * np.pi * x)
        return f(soucetmocnin,soucetcosinu)
    def __init__(self):
        self.lb=-32.768
        self.ub= 32.768
...
class ZakharovFunction(Function):
    ...
    def zakharovN(self,params):
        ...
        for x in params:
            ...
            soucetnasobku+=x*x
        a= 0.5*soucet
        b= soucetnasobku +pow(a,2) +pow(a,4)
        return b
    def __init__(self):
        self.lb=-5
        self.ub= 10
...
class MichalewiczFunction(Function):
    ...
    def useFn(self,params,ign):
        ...
        soucet=0
        for x in params:
            soucet+=(np.sin(x) * np.sin((1 * x**2) / np.pi)**20)
        return -1 * soucet
...
class LevyFunction(Function):
    ...
    def cZN(self,params):
        ...
        for i in w:
            s += (i - 1)**2 * (1 + 10 * np.sin(np.pi * i + 1)**2)
        return np.sin(np.pi * w[0])**2 + s + (w[n - 1] - 1)**2 * (1 + np.sin(2 * np.pi * w[n - 1]))**2
    def useF(self,x,y):
        return self.cZ(x,y)
    def useFn(self,params,ign):
...
class GriewankFunction(Function):
    ...
    def useFn(self,params,ign):
        ...
        for x in params:
            soucetnas+=x**2
            nasod*=np.cos(x/np.sqrt(count))
            count+=1

        f=lambda soucet,nas: soucet/4000 -nas+1
        return f(soucetnas,nasod)
...
class RastriginFunction(Function):
    ...
    def useFn(self,params,ign):
        ...
        soucet=0
        for x in params:
            soucet+=x**2 - 10* np.cos(2 * np.pi * x)
        return 10+soucet
